                ...
                if (!firstCard) {
                    ...
                    if (firstCardValue == secondCardValue) {
                        ...
                        if (winCount == Math.floor(cardValues.length / 2)) {
                            result.innerHTML = `<h2>Congratulation You have match all the cards</h2>
...
submitButton.addEventListener('click', function (event) {
    ...
    let name = nameInput.value; // get name from input
    displayName.textContent = name; // display name in game
    localStorage.setItem('name', name); // store name in local storage
});

const storedName = localStorage.removeItem('name'); // get name from local storage
if (storedName) {
    displayName.textContent = storedName; // display name in game
}
...
toggleMusicButton.addEventListener("click", function () {
    if (backgroundMusic.paused) {
        backgroundMusic.play();
        toggleMusicButton.textContent = "Turn off audio";
        localStorage.setItem("backgroundMusic", backgroundMusic.play());
    } else {
        backgroundMusic.pause();
        toggleMusicButton.textContent = "Turn on Audio";
        localStorage.setItem("backgroundMusic", backgroundMusic.paused);
    }
});
